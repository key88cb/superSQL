...
int BufferManager::unpinPage(std::string file_name, int block_id) {
    int page_id = getPageId(file_name, block_id);
    if (page_id == -1) return -1;
...
int BufferManager::getEmptyPageId() {
    ...
    int iter_count = 0;
    while (1) {
        if (iter_count > frame_size_ * 2) {
            std::cerr << "Fatal Error: Buffer Manager is full and all pages are pinned." << std::endl;
            exit(1);
        }
...
int BufferManager::getPinnedCount() {
    int count = 0;
    for (int i = 0; i < frame_size_; i++) {
