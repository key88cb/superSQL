...
RecordManager::~RecordManager() {
    for (auto it = index_managers.begin(); it != index_managers.end(); ++it) {
        delete it->second;
    }
    index_managers.clear();
}

IndexManager* RecordManager::getIndexManager(std::string table_name) {
    if (index_managers.find(table_name) == index_managers.end()) {
        index_managers[table_name] = new IndexManager(table_name);
    }
    return index_managers[table_name];
}

void RecordManager::invalidateIndexManager(std::string table_name) {
    if (index_managers.find(table_name) != index_managers.end()) {
        delete index_managers[table_name];
        index_managers.erase(table_name);
    }
}
...
void RecordManager::insertRecord(std::string table_name , Tuple& tuple) {
    ...
    for (int i = 0;i < attr.num;i++) {
        if (attr.primary_key == i || attr.unique[i] == true) {
            if (attr.has_index[i]) {
                std::string file_path = "INDEX_FILE_" + attr.name[i] + "_" + tmp_name;
                IndexManager* im = getIndexManager(tmp_name);
                if (im->findIndex(file_path, v[i]) != -1) {
                    if (attr.primary_key == i) throw primary_key_conflict();
                ...
                Table t = selectRecord(tmp_name, attr.name[i], w);
                if (t.getTuple().size() > 0) {
                    if (attr.primary_key == i) throw primary_key_conflict();
                    else throw unique_conflict();
                }
            }
    ...
    //如果剩余的空间不够
        ...
        buffer_manager.unpinPage(table_name, block_num - 1);
    ...
    //更新索引
    IndexManager* index_manager = getIndexManager(tmp_name);
    for (int i = 0;i < attr.num;i++) {
        ...
        while(*p != '\0' && p < t + PAGESIZE) {
            ...
            for (int j = 0;j < attr.num;j++) {
                if (attr.has_index[j] == true) {
                    std::string attr_name = attr.name[i]; // wait, attr.name[j]? 
                    std::string file_path = "INDEX_FILE_" + attr_name + "_" + tmp_name;
                    std::vector<Data> d = tuple.getData();
...
Table RecordManager::selectRecord(std::string table_name , std::string result_table_name) {
    ...
    for (int i = 0;i < block_num;i++) {
        ...
        buffer_manager.unpinPage(table_name, i);
    }
    return table;
}
...
int RecordManager::getBlockNum(std::string table_name) {
    ...
    do {
        ...
        block_num++;
        go_on = (p[0] != '\0');
        buffer_manager.unpinPage(table_name, block_num);
    } while(go_on);
    return block_num;
}
...
void RecordManager::searchWithIndex(std::string table_name , std::string target_attr , Where where , std::vector<int>& block_ids) {
    IndexManager* index_manager = getIndexManager(table_name);
    Data tmp_data;
...
void RecordManager::conditionSelectInBlock(std::string table_name , int block_id , Attribute attr , int index , Where where , std::vector<Tuple>& v) {
    ...
    buffer_manager.unpinPage(table_name, block_id);
}
