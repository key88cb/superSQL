...
void Table::showTable(){
    ...
    std::cout<<std::endl;
    for(size_t index=0;index<tuple_.size();index++)
...
void Table::showTable(int limit) {
    for(int index=0;index<attr_.num;index++)
        std::cout<<attr_.name[index]<<'\t';
    std::cout<<std::endl;
    for(int index=0;index<limit&&(size_t)index<tuple_.size();index++)
        tuple_[index].showTuple();
}
