...
- Leader 选举已补充抖动混沌验证：`LeaderElectorTest` 增加连续主节点 flapping 场景，验证多轮切换后始终单主且 `/active-master.epoch` 严格单调递增。
- Leader 判定异常已收敛为 fail-closed：`/active-master` 读取失败、空载荷或损坏时按 standby 处理（返回 `NOT_LEADER`），避免异常状态误放行写入。
