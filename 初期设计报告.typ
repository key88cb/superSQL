...
- `/region*servers`：临时节点，每个 Region Server 启动时创建，内容为 RS 的 IP:Port 及当前表数量。Master 监听此目录以感知节点加入/退出。
- `/meta/tables`：持久节点，存储所有表的元数据（表名 → 主副本所在的 Region Server）。Master 更新此节点后，Client 可通过读取此处获得路由信息。
- `/assignments`：持久节点，记录每个表的副本分布（例如 `table*t1` 节点内容为 `["rs1:9090", "rs2:9090", "rs3:9090"]`）。

Watcher 机制：Master 在 `/region*servers` 上设置 Watcher，当节点消失时立即触发重新分配 Region 的流程。

=== Master 服务器（多 Master 高可用设计）

*领导者选举*：启动时在 Zookeeper `/masters` 下创建临时顺序节点，序号最小的节点成为 Active Master。Active Master 每 5 秒向 `/master*active` 写入心跳时间戳。Standby Master 监听此节点，若 15 秒未更新则发起重新选举。
...
B+ 树中索引节点大小应与缓冲区的块大小相同(4096KB)，B+ 树的叉数由节点大小与索引键大小计算得到。
...
- *强一致写*：客户端同时向 3 个副本发送写请求，全部成功才返回。
...
#align(center)[
  #table(
    ...
    [06], [SQL 脚本执行], [exefile test.sql], [依次执行文件中的 10 条 SQL], [验证批量执行],
