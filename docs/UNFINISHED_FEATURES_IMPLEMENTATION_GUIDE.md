...
- RegionServer：迁移协议主链路已完整化（块级签名校验 + manifest 失败回滚清理），后续重点转向大文件/长时中断/跨版本兼容场景的压测与混沌验证。
...
- `getTableLocation/listTables/repairTableRoutesWithConfirmation` 已接入 `recoverStuckMigrationWithConfirmation`：当迁移状态超时且 `migrationAttemptId` 仍存在时，恢复会先走补偿确认，再决定是否切回 `ACTIVE`（`repairTableRoutesBestEffort` 仅保留为兼容入口）。
...
- `RegionMigrator` 已补充跨节点补偿恢复策略：当 source/target 补偿副本缺失或不可解析时，会尝试在“非 assignment 在线节点”执行补偿清理确认并收敛回 `ACTIVE`。
- Leader 选举已补充“重复主节点抖动（flapping）”混沌验证：连续触发多轮主备切换，验证 `active-master` 收敛一致且 `epoch` 严格单调递增。
...
- 无（本轮已补齐主从切换抖动场景的混沌验证；Master 侧该项收口）。
...
- RegionServer `/status` 已补充 `prepareDecision` 与 `replicaCommitRetry` 运行态统计，用于观察 PREPARE 决议与提交通知重试收敛；`replicaCommitRetry` 已包含修复触发/成功/失败与错误分类分布。
...
- 自治恢复：主副本晋升、补副本、恢复后自动追赶到一致状态；其中 S6-05 已补上 suspected 副本检测/状态暴露/恢复统计的验证入口，但“Master 自动摘除并补建替换副本”的闭环仍未完成。
